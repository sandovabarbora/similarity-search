...
class H5FileManager:
    ...
    def append_features(
        self, features: np.ndarray, paths: List[str], metadata: Dict[str, Any] = None
    ):
        """
        ...
        with h5py.File(self.filename, self.mode) as h5_file:
            ...
            if "features" in h5_file:
                ...
                # Resize and append
                new_feature_shape = (
                    existing_features.shape[0] + features.shape[0],
                    features.shape[1],
                )
                ...
                h5_file.create_dataset("paths", data=paths, dtype=dt, maxshape=(None,))
    ...
    def get_library_versions(self) -> Dict[str, str]:
        ...
        """
        libraries = ["numpy", "torch", "torchvision", "h5py", "tqdm", "psutil", "Pillow"]
    ...
    def parallel_scan(self, root_dir: Path, num_workers: int) -> List[Path]:
        ...
        try:
            ...
            with ProcessPoolExecutor(max_workers=num_workers) as executor:
                ...
                for future in tqdm(
                    futures, desc="Scanning directories", total=len(chunks), unit="chunk"
                ):
...
class ImageFeatureExtractor:
    ...
    def process_batch(
        self, batch_tensors: torch.Tensor, batch_paths: List[str]
    ...
    def batch_extract_features(
        self, image_paths: List[Union[str, Path]], batch_size: int = 64
    ) -> Tuple[np.ndarray, List[str]]:
        ...
        try:
            ...
            with tqdm(
                ...
                for batch_tensors, batch_paths in dataloader:
                    ...
                    pbar.update(1)
                    pbar.set_postfix(
                        {"processed_images": len(valid_paths), "batch_size": len(batch_paths)}
                    )
...
def process_image_dataset(
    ...
    try:
        ...
        features, valid_paths = extractor.batch_extract_features(
            new_image_paths, batch_size=batch_size
        )
        ...
        logger.info(
            f"{'Appending' if append else 'Saving'} {len(valid_paths)} new features to {output_file}"
        )
...
def main():
    ...
    parser.add_argument(
        "--append", action="store_true", help="Append to existing H5 file instead of overwriting"
    )
