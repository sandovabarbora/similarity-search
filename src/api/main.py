...
def init_background():
    ...
    try:
        ...
        with h5py.File(h5_path, 'r') as f:
            ...
            logger.info(f"Dataset has {image_count} images with {image_feature_dimension} dimensions")
        ...
        if use_optimized_search:
            try:
                ...
                logger.error(f"Error initializing optimized search: {e}. Falling back to original search.")
                use_optimized_search = False
...
def load_path_cache(h5_path):
    ...
    try:
        ...
        with h5py.File(h5_path, 'r') as f:
            if 'paths' not in f:
                logger.error(f"'paths' dataset not found in H5 file. Available keys: {list(f.keys())}")
        ...
        logger.info(f"Path cache loaded in {time.time() - start_time:.2f}s, {len(path_lookup)} entries")
        
        # Log some example paths for debugging
        num_examples = min(5, len(path_lookup))
        for i in range(num_examples):
            logger.info(f"Path example {i}: {path_lookup[i]}, exists: {os.path.exists(path_lookup[i])}")
        ...
        if use_optimized_search:
            try:
                ...
                for item in similar_images:
                    ...
                    req_logger.debug(f"Path: {item['path']}, exists: {os.path.exists(item['path'])}")
                
                return {"status": "success", "results": similar_images}
            except Exception as e:
                # Fall back to original search if optimized fails
                req_logger.warning(f"Optimized search failed: {e}. Falling back to original search.")
                use_optimized_search = False
...
async def get_stats():
    ...
    if not api_ready:
        return {
            ...
            "eta_seconds": 5  # Estimated time remaining
        }
        
    try:
        # Get sample paths
        sample_paths = []
        if use_optimized_search and search_engine and hasattr(search_engine, 'paths') and search_engine.paths:
            sample_paths = [ensure_valid_path(p) for p in search_engine.paths[:5]]
        elif path_cache_loaded:
            sample_paths = [path_lookup.get(i, f"image_{i}") for i in range(min(5, len(path_lookup)))]
        ...
        for path in sample_paths:
            ...
            logger.debug(f"Sample path: {path} -> {valid_path}, exists: {os.path.exists(valid_path)}")
