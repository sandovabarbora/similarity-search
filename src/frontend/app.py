...
def load_explore_data():
    ...
    if st.session_state.explore_data is None:
        ...
        recommendations = get_recommendations(
            user_likes=st.session_state.likes,
            user_saves=st.session_state.saved
        )
    ...
    with tab1:
        ...
        if valid_samples and len(valid_samples) >= 2:
            ...
            with feat_col1:
                ...
                # Like button
                if st.button("❤️" if sample1 in st.session_state.likes else "🤍", key=f"like_feat_1"):
                    toggle_like(sample1)
                    st.rerun()
            ...
            with feat_col2:
                ...
                # Like button
                if st.button("❤️" if sample2 in st.session_state.likes else "🤍", key=f"like_feat_2"):
                    toggle_like(sample2)
                    st.rerun()
        else:
            ...
            with feat_col1:
                st.image("https://images.unsplash.com/photo-1486406146926-c627a92ad1ab?ixlib=rb-4.0.3&auto=format&fit=crop&w=600&q=80")
                st.markdown("**Design Excellence**")
                st.write("Upload your design to find similar inspirations")
            
            with feat_col2:
                st.image("https://images.unsplash.com/photo-1531403009284-440f080d1e12?ixlib=rb-4.0.3&auto=format&fit=crop&w=600&q=80")
                        ...
                        with row_cols[j]:
                            ...
                            try:
                                if os.path.exists(item['path']):
                                    ...
                                    st.image(f"https://picsum.photos/seed/explore{idx}/300", use_container_width=True)
                            except:
                                st.image(f"https://picsum.photos/seed/explore{idx}/300", use_container_width=True)
                            ...
                            # Like button
                            if st.button("❤️" if item['path'] in st.session_state.likes else "🤍", key=f"like_explore_{idx}"):
                                toggle_like(item['path'])
                                st.rerun()
                            
                            # Save button - separate line to avoid nested columns error
                            if st.button("📌" if item['path'] in st.session_state.saved else "📍", key=f"save_explore_{idx}"):
        ...
        with col1:
            ...
            if uploaded_file:
                ...
                try:
                    ...
                    for i, filter_name in enumerate(filter_options):
                        with filter_cols[i % 3]:
                            if st.button(
                                ...
                                type="primary" if filter_name == st.session_state.applied_filter else "secondary"
                            ):
                    ...
                    st.image(filtered_image, caption=f"{st.session_state.applied_filter} Filter", use_container_width=True)
            ...
            if st.session_state.show_results and 'search_results' in st.session_state:
                ...
                if results and len(results) > 0:
                    ...
                    if valid_path and os.path.exists(valid_path):
                        ...
                        # Like button
                        if st.button("❤️" if valid_path in st.session_state.likes else "🤍", key=f"like_result_0"):
                            toggle_like(valid_path)
                            st.rerun()
                        
                        # Save button
                        if st.button("📌" if valid_path in st.session_state.saved else "📍", key=f"save_result_0"):
                            ...
                            if idx < len(results):
                                ...
                                with cols[j]:
                                    if valid_path and os.path.exists(valid_path):
                                        ...
                                        # Like button
                                        if st.button("❤️" if valid_path in st.session_state.likes else "🤍", key=f"like_result_{idx}"):
                                            toggle_like(valid_path)
                                            st.rerun()
                                        
                                        # Save button
                                        if st.button("📌" if valid_path in st.session_state.saved else "📍", key=f"save_result_{idx}"):
                                            toggle_save(valid_path)
                                            st.rerun()
            else:
